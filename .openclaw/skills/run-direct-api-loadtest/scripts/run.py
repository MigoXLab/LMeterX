#!/usr/bin/env python3
"""
run-direct-api-loadtest skill — thin client for LMeterX backend.
...
Workflow:
  ...
  3. Pre-check connectivity via /api/tasks/test or /api/common-tasks/test
  4. Create loadtest task via /api/tasks or /api/common-tasks
...
from _lib.client import ARTIFACT_ROOT, BASE_URL, headers  # noqa: E402
...
def _run_llm_flow(
    ...
    """
    LLM API flow:
      1. POST /api/tasks/test  → connectivity check
      2. POST /api/tasks       → create loadtest task
    """
    ...
    try:
        ...
        if test_data.get("status") != "success":
            print(f"\n❌ 连通性测试未通过:")
            print(f"   状态: {test_data.get('status', 'unknown')}")
            print(f"   错误: {test_data.get('error', 'N/A')}")
            return None

        http_status = test_data.get("http_status", "?")
        print(f"   ✅ 连通性正常 → HTTP {http_status}")
...
def _run_common_flow(
    ...
    """
    Common HTTP API flow:
      1. POST /api/common-tasks/test  → connectivity check
      2. POST /api/common-tasks       → create loadtest task
    """
    print(f"   请求方法: {method}")
    print(f"   目标 URL: {url}")
    ...
    try:
        test_resp = client.post(
            f"{BASE_URL}/api/common-tasks/test",
            headers=headers(),
            json=test_payload,
        )
