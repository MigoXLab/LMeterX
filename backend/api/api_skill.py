...
Exposes three key endpoints for external agent skills:
  ...
  2. POST /api/http-tasks/test      — Test API connectivity (already exists)
  3. POST /api/http-tasks           — Create load test task (already exists)
...
async def analyze_url(request: Request, body: AnalyzeUrlRequest):
    ...
    ``POST /api/http-tasks`` one by one to create loadtest tasks.
    """
