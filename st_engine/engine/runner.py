...
class LocustRunner:
    ...
    def _run_warmup_phase(self, task: Task, task_logger) -> None:
        ...
        try:
            warmup_process = subprocess.Popen(
                ...
                # Keep Locust stderr and application stdout in one ordered
                # stream; separate reader threads can otherwise interleave them.
    ...
    def _start_process(
        ...
        process = subprocess.Popen(
            cmd,
            stdout=subprocess.PIPE,
            # Locust emits table rows on stderr and task lifecycle logs on
            # stdout.  A shared pipe preserves their actual child write order.
            stderr=subprocess.STDOUT,
            text=True,
            bufsize=1,
            env=env,
